...
int integer_node::attrib(int)
{
	...
	if (value_tkn->in_text[0] == '0'
		&& (value_tkn->in_text[1] == 'x' || value_tkn->in_text[1] == 'X'))
	{
		sscanf(value_tkn->in_text + 2, "%x", &value);
		radix = 16;
	}
	else if (turbo_pascal && *value_tkn->in_text == '$') {
		sscanf(value_tkn->in_text + 1, "%x", &value);
		radix = 16;
	}
	else if (strncmp(value_tkn->in_text, "2#", 2) == 0) {
		value = btoi(value_tkn->in_text + 2);
		radix = 8;
	}
	else if (strncmp(value_tkn->in_text, "8#", 2) == 0) {
		sscanf(value_tkn->in_text + 2, "%o", &value);
		radix = 8;
	}
	else if (strncmp(value_tkn->in_text, "16#", 3) == 0) {
		sscanf(value_tkn->in_text + 3, "%x", &value);
		radix = 16;
	}
	else if (strncmp(value_tkn->in_text, "10#", 3) == 0) {
		sscanf(value_tkn->in_text + 3, "%d", &value);
		radix = 10;
	}
	else if (!isdigit(value_tkn->in_text[strlen(value_tkn->in_text) - 1])) {
		sscanf(value_tkn->in_text, "%o", &value);
		radix = 8;
	}
	else {
...
int deref_expr_node::translate(int ctx)
{
	...
	l_tkn = op;
	op->disable();
	if (ptr->type->tag == tp_file || ptr->type->tag == tp_text) {
		tag = tn_filevar;
		if (ctx != ctx_lvalue) {
			if (language_c) {
				ptr->l_tkn->append(")");
				if (ctx == ctx_access) {
					f_tkn = f_tkn->prepend("access(");
					tag = tn_address;
				}
				else {
					if (ctx == ctx_array) {
...
int access_expr_node::translate(int ctx)
{
	...
	else if (rec->type != NULL && (rec->type->tag == tp_unit || rec->type->tag == tp_object)) {
		pnt->set_trans("::");
	}
					...
					if (args->type->is_reference()) {
						...
						if (ctx == ctx_rvalue) {
							if (args->next != NULL) {
								...
								token::remove(lpar, rpar);
								l_tkn = f_tkn;
								f_tkn->set_trans(dprintf("new %s", cls->name));
							}
						}
						else {
							args->f_tkn->set_pos(f_tkn);
	...
	if (fptr->type && fptr->type->tag == tp_proc) {
		...
		for (expr_node* e = args; e != NULL; e = e->next) {
			if (p != NULL) {
				...
				if (p->var->type->is_array() && e->type->tag == tp_char) {
					if (e->tag == tn_char) {
